...
function makeCurrencyString(currencies, capital) {
    if (currencies.length === 1) return `<p>The capital is ${capital} and you can pay with ${currencies[0].name}</p>`;
...
async function searchCountry(e) {
    try {
        ...
        // Make API call
        const results = await _axiosDefault.default.get(`https://restcountries.com/v2/name/${e}`);
    ...
    // Catch errors and inject error's to the page
        ...
        if (document.getElementById('search-field').value.length === 0) document.getElementById('error').innerText = 'Vul een waarde in';
        if (e1.response.status === 404 && document.getElementById('search-field').value.length !== 0) document.getElementById('error').innerText = 'Geen resultaten gevonden';
...
//# sourceMappingURL=country-searcher.335cc6be.js.map
